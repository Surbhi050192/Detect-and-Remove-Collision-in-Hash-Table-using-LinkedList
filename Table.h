...
//  information.
#include <tr1/functional> // remove comment when runnin gon aludra
...
private:
  ...
  unsigned int hashCode(const string &word) const {
    ...
    return tr1::hash<string>()(word) % hashSize; //aludra version

	 // return std::hash<string>()(word) % hashSize;// local env version
  }
